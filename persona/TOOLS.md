...
#### Files, Notes & Design
- **Google Drive** (`google-drive-api`): School projects, freelance deliverables, Lantern Tides design documents, and meeting notes. The shared-files surface.
- **Dropbox** (`dropbox-api`): Greenleaf Youth Arts hands off brand assets through Dropbox; mirror new drops into Drive, keep the source folder shared with Zoe.
...
- **Figma** (`figma-api`): Primary design surface for UX work and pixel-art mockups. Files routed through Drive when shared with clients.
...
- **Kubernetes** (`kubernetes-api`): Read-only access to the Thornfield research cluster Ravi gave her for the Lantern Tides audio-processing pipeline.
- **Cloudflare** (`cloudflare-api`): Fronts `angelabrooks.design` portfolio DNS. Surface status only when the site goes down.
...
- **Jira** (`jira-api`): Harborview's small dev team kept the booking-flow refactor in Jira; she still has guest access to update the design spec ticket.
...
- **Discord** (`discord-api`): Indie dev community and game jam team channels. Read context only. Do not post on her behalf.
...
- **Pinterest** (`pinterest-api`): Reference boards for pixel art, type studies, and Lantern Tides mood. Read-only.
...
- **Alpaca** (`alpaca-api`): Paper-trading account she set up to learn options after a finance-podcast deep dive; still paper, no real money in.
...
- **Reddit** (`reddit-api`): r/gamedev and the design subreddits, read-only. She researches purchases there before she buys.
- **Twitter** (`twitter-api`): Indie dev community and a small mutuals list. Read-only. Do not post on her behalf.
...
- **NASA** (`nasa-api`): Reference for the night-sky pixel scenes in Lantern Tides chapter four. Visual sourcing only.
